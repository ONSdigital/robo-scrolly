<script>
  ...
  function formatData(data) {
      ...
      if (data[0].z) {
          ...
          const xDomain = Array.from(new Set(data.map(d => d.x))).sort((a, b) => a.localeCompare(b));
          csv = `"Area name",${xDomain.join(",")}\n`;
          for (const key in indexed) {
              const d = indexed[key];
              csv += `"${d.name}",${xDomain.map(x => d[x]).join(",")}\n`
          }
      } else {
          csv = '"Area name",value\n';
          for (const d of data) {
  ...
  function downloadData() {
      let csv = section.title ? `"${section.title}"\n` : "";
      csv += section.subtitle ? `"${section.subtitle}"\n` : "";
      if (csv) csv += "\n"
